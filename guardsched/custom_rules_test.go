...
func TestParseRulesText_ExampleRules(t *testing.T) {
	zonesRaw, err := os.ReadFile(filepath.Join("..", "zones-example.yaml"))
	if err != nil {
...
func TestResolveSlotID_NameAnd1Based(t *testing.T) {
	...
	idx, err := ResolveSlotID(zc, "gate_north_9")
	if err != nil || idx != 0 {
		t.Fatalf("gate_north_9 → 0, got %d err=%v", idx, err)
	}
