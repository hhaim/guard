...
func TestExampleHotDay4AtMostOneRotatingPerSoldier(t *testing.T) {
	root := filepath.Join("..")
	zonesRaw, err := os.ReadFile(filepath.Join(root, "zones-example.yaml"))
	if err != nil {
		t.Skip("zones-example.yaml not in repo root")
	}
	rosterRaw, err := os.ReadFile(filepath.Join(root, "roster-example.yaml"))
	if err != nil {
		t.Skip("roster-example.yaml not in repo root")
	}
