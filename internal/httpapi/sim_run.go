...
func (s *Server) runScheduleSimulation(ctx context.Context, body scheduleRunBody, debugOffset int, slot string) (*simRunOutput, *APIErrorBody) {
	...
	hasHistory := prefix.Days > 0 && len(prefix.Records) > 0
	useWitnessExtend := hasHistory &&
		prefix.Continuation != nil && prefix.Continuation.RNGState != nil && !expertRulesActive
	availHorizonDays := planAvailHorizonDays(planDays, prefix.Days, hasHistory, useWitnessExtend)
