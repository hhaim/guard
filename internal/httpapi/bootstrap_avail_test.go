...
func TestBootstrapColdFailsWhenAvailabilityScopedToPlanDaysOnly(t *testing.T) {
	root := findRepoRoot(t)
	zonesRaw, err := os.ReadFile(root + "/zones-dv3.yaml")
	if err != nil {
		t.Skip("zones-dv3.yaml missing")
	}
	rosterRaw, err := os.ReadFile(root + "/roster-dv3.yaml")
	if err != nil {
		t.Skip("roster-dv3.yaml missing")
	}
...
func TestPlanAvailHorizonForBootstrapCold(t *testing.T) {
	planDays := 1
	prefixDays := 14
	expertRulesActive := true
	continuationLoaded := true
	useWitnessExtend := prefixDays > 0 && continuationLoaded && !expertRulesActive
	useBootstrapCold := prefixDays > 0 && !useWitnessExtend
	availHorizon := planDays
	if useBootstrapCold {
		availHorizon = prefixDays + planDays
	}
	if availHorizon != 15 {
		t.Fatalf("want avail horizon 15 for bootstrap cold, got %d", availHorizon)
	}
}
